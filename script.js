...
const ECOLS = ['plate','car-type','wash-type','box','cost','loyalty','phone'];
const EMPTY_ROWS_BUFFER = 25; // always keep this many blank rows at bottom

// ── STATE ─────────────────────────────────────────────────────
const S = {
  managerName : '',
  lists       : null,
  savedRows   : [],       // entries from GAS (today's shift)
  statsTimer  : null,
  webAppUrl   : '',
  collectingRowIdx : null,  // GAS rowIndex for pending collect from log
  collectPay       : null,
  editingRowIdx    : null
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bindEditModal();
  addInvRow();
  // Render placeholder grid immediately
  prerenderGrid();
});
...
function initApp(){
  google.script.run
    .withSuccessHandler(url=>{S.webAppUrl=url;const el=document.getElementById('live-link-btn');if(el&&url)el.href=url+'?page=live';})
    .withFailureHandler(()=>{})
    .getWebAppUrl();
...
function makeFilledRow(r, rowNum){
  ...
  const payBadge=isPending
    ?'<span class="pay-badge pb-pending">⏳ ტაბი</span>'
    :`<span class="pay-badge pb-${(r.paymentType||'').toLowerCase()}">${esc(r.paymentType)}</span>`;
  const actionBtn=isPending
    ?`<button class="row-collect-btn" onclick="openCollectFromLog(${r.rowIndex},'${esc(r.plateNumber)}',${r.cost})">💰 Collect</button>`
    :`<button class="row-edit-btn" onclick="openEditFromLog(${r.rowIndex})">✏</button>`;
  ...
  tr.innerHTML=`
    ...
    <td><select data-col="box"       onchange="onBoxOrTypeChange(this)" onkeydown="onRowKey(event,this)">${boxOpts(r.box)}</select></td>
    ...
    <td class="col-ro" style="text-align:center">${esc(r.timestamp)}</td>
    <td class="col-ro" style="text-align:center">${payBadge}</td>
...
function makeEmptyRow(rowNum){
  ...
  tr.innerHTML=`
    ...
    <td><select data-col="box"       onchange="onBoxOrTypeChange(this)" onkeydown="onRowKey(event,this)">${boxOpts()}</select></td>
...
// ── BOX / TYPE CHANGE → update colour + auto-price ───────────
function onBoxOrTypeChange(sel){
  const tr=sel.closest('tr');
  autoPrice(tr);
  if(sel.dataset.col==='box'){
    // Update row colour
    tr.classList.remove('bx-1','bx-2','bx-3','bx-4');
  ...
  if(sel.dataset.col==='wash-type'){
    const vipCell=tr.querySelector('[data-col="wash-type"]').closest('td');
    if(sel.value==='VIP') vipCell.classList.add('vip-cell');
    else                   vipCell.classList.remove('vip-cell');
...
function carOpts(sel){
  return CAR_TYPES.map(v=>`<option${v===sel?' selected':''}>${v}</option>`).join('');
}
function washOpts(sel){
  return WASH_TYPES.map(v=>`<option${v===sel?' selected':''}>${v}</option>`).join('');
}
function boxOpts(sel){
  return BOXES.map(v=>`<option${v===sel?' selected':''}>${v}</option>`).join('');
}
