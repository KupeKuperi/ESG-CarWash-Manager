...
function geoDateStr(d) {
  return d.getDate() + ' ' + GEO.months[d.getMonth()] + ', ' + d.getFullYear();
}
...
// ── GLOBAL CLOCK (runs always, even on login screen) ──────────
function startGlobalClock() {
  function tick() {
    ...
    // Header clock (main app)
    setEl('hdr-hh', hh); setEl('hdr-mm', mm); setEl('hdr-ss', ss);
    setEl('header-date', GEO.days[now.getDay()] + ', ' + geoDateStr(now));

    // Start-shift clock
    setEl('ss-hh', hh); setEl('ss-mm', mm); setEl('ss-ss', ss);
    setEl('ss-day-name', GEO.days[now.getDay()]);
    setEl('ss-date-str', geoDateStr(now));
  }
  tick();
...
function onLoginSuccess(name) {
  ...
  // Show START SHIFT screen (not the app yet)
  document.getElementById('login-screen').style.display = 'none';
  const ssScreen = document.getElementById('start-shift-screen');
  ssScreen.classList.add('visible');

  // Populate start-shift card
...
function bindStartShift() {
  ...
  btn.addEventListener('click', () => {
    ...
    google.script.run
      .withSuccessHandler(res => {
        setLoading(btn, false);
        // Hide start-shift screen, show main app
        document.getElementById('start-shift-screen').classList.remove('visible');
        document.getElementById('app').classList.add('visible');
        document.getElementById('manager-name-display').textContent = STATE.managerName;

        // Load lists, entries, stats
        google.script.run
        ...
        toast('✓ ცვლა დაიწყო — ' + new Date().toLocaleTimeString('ka-GE',{hour:'2-digit',minute:'2-digit'}), 'success');
      })
      .withFailureHandler(err => {
        setLoading(btn, false);
        toast('შეცდომა: ' + err.message, 'error');
      })
      .setShiftStart(STATE.managerName);
  });
}
...
function logout() {
  ...
  // Show login, hide app and start-shift screen
