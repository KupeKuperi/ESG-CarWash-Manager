...
function autoSyncGrid(){
  document.querySelectorAll('#wash-tbody tr').forEach(tr=>{
    ...
    const cost=parseFloat(tr.querySelector('[data-col="cost"]')?.value)||0;
    if(plate && cost>0) submitNewRow(tr, plate, {rebuild:false, toast:false});
  });
}
...
function onPaymentChange(sel){
  ...
  if(val){
    ...
    // ── NEW unsaved row: save it now with the selected payment ──
    if(tr.dataset.state==='new' && plate && cost>0){
    ...
    if(tr.dataset.state==='pending' && !isNaN(rowIdx)){
      google.script.run
        .withSuccessHandler(res=>{
          if(res.success){
    ...
    .withSuccessHandler(res=>{
      if(res.success){
        if(rebuild){
          ...
          // In-place: preserve cursor, just mark the row as saved
          tr.dataset.state  = data.status==='Paid'?'paid':'pending';
